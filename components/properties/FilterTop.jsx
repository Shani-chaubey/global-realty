...
export default function FilterTop() {
  ...
  const typeLabel = useMemo(() => {
    const t = propertyTypes.find((p) => p._id === type);
    return t?.name || "Type";
  }, [propertyTypes, type]);
  ...
  const handleSearch = (e) => { if (e?.preventDefault) e.preventDefault(); applyFilters(); };

  const clearAll = () => {
    setQ(""); setStatus(""); setType(""); setBaths(""); setBeds("");
    setCity(""); setListingType(""); setPossessionStatus("");
    setAmenities([]); setPriceRange([0, PRICE_MAX]); setAreaRange([0, AREA_MAX]);
    setPanelOpen(false);
  ...
  const removeFilter = (key, value) => {
    ...
    if (key === "amenities") {
      ...
      next.length ? params.set("amenities", next.join(",")) : params.delete("amenities");
    } else {
      params.delete(key);
    }
  ...
  const activeFilters = useMemo(() => {
    ...
    if (listingType)     chips.push({ key: "listingType",     label: `For: ${listingType}` });
    if (possessionStatus)chips.push({ key: "possessionStatus",label: `Possession: ${possessionStatus}` });
    if (priceRange[0] > 0 || priceRange[1] < PRICE_MAX)
      chips.push({ key: "price", label: `Price: ${fmt(priceRange[0])} – ${fmt(priceRange[1])}` });
    if (areaRange[0] > 0 || areaRange[1] < AREA_MAX)
      chips.push({ key: "area", label: `Area: ${areaRange[0]}–${areaRange[1]} sqft` });
    amenities.forEach((slug) => {
      const a = amenityList.find((x) => toSlug(x.name) === slug);
      chips.push({ key: "amenities", value: slug, label: a?.name || slug });
    });
    return chips;
  }, [q, status, type, typeLabel, beds, baths, city, listingType, possessionStatus, priceRange, areaRange, amenities, amenityList]);
          ...
          <div className="col-lg-12">
            <div className="title-inner">
              <ul className="breadcrumb">
                <li>
                  <Link className="home fw-6 text-color-3" href="/">Home</Link>
                </li>
                <li>Properties</li>
              </ul>
            </div>
              ...
              <div className="form-title style-2">
                ...
                <div className="wrap-btn">
                  <div
                    ...
                    title="More filters"
                    style={{ background: panelOpen ? "var(--Sub-primary-2)" : undefined }}
                  >
                  ...
                  <button type="button" onClick={handleSearch} className="tf-btn bg-color-primary pd-3 fw-6">
              ...
              >
                ...
                <div className="group-select">
                  <div className="box-select">
                    <DropdownSelect
                      options={["City", "Mumbai", "Delhi", "Bangalore", "Hyderabad", "Pune", "Chennai", "Kolkata", "Ahmedabad", "Jaipur"]}
                  ...
                  <div className="box-select">
                    <DropdownSelect
                      ...
                      onChange={(v) => setListingType(v === "Listing Type" ? "" : v)}
                      addtionalParentClass=""
                    />
                  </div>
                  <div className="box-select">
                    <DropdownSelect
                      ...
                      onChange={(v) => setPossessionStatus(v === "Possession" ? "" : v)}
                      addtionalParentClass=""
                    />
                  </div>
                </div>
                ...
                <div style={{ display: "flex", gap: 12, marginTop: 20, paddingTop: 16, borderTop: "1px solid var(--Line)" }}>
                  <button type="button" onClick={applyFilters} className="tf-btn bg-color-primary pd-3 fw-6">
                    Apply Filters
                  </button>
                  <button type="button" onClick={clearAll} className="tf-btn style-1 pd-3 fw-6">
                    Reset All
                  </button>
                </div>
              </div>
