...
const normalizeImages = (images = [], title = "Property") => {
  if (!Array.isArray(images) || images.length === 0) {
    return FALLBACK.map((url) => ({ url, alt: title }));
  }
  const result = images
    .map((img) => {
      if (typeof img === "string") return { url: img, alt: title, isPrimary: false };
      if (img && typeof img === "object") {
        const url = img.url || img.src || img.image || img.imageUrl || img.secure_url || "";
        return { url, alt: img.alt || title, isPrimary: Boolean(img.isPrimary) };
      }
      return null;
    })
    .filter((img) => img?.url)
    .sort((a, b) => Number(b.isPrimary) - Number(a.isPrimary));
  return result.length ? result : FALLBACK.map((url) => ({ url, alt: title }));
};

export default function Slider1({ images = [], title = "Property" }) {
  const all = normalizeImages(images, title);

  // Ensure we have at least 4 slots using fallbacks
  const slots = [...all];
  while (slots.length < 4) slots.push({ url: FALLBACK[slots.length], alt: title });

  const [img1, img2, img3, img4] = slots;
  const extra = all.slice(4); // hidden gallery items for lightbox count
    ...
    <section id="gallery-swiper-started" className="section-property-image">
      <div className="tf-container">
        <Gallery>
          <div className="row">
            <div className="col-12">
              <div className="wrap-image">
                  ...
                  >
                    {({ ref, open }) => (
                      <a
                        ...
                        className="image-wrap relative d-block h-full"
                      >
                        <Image
                          ref={ref}
                          className="lazyload object-cover h-full"
                          data-src={img1.url}
                          alt={img1.alt}
                          src={img1.url}
                          width={1095}
                ...
                {/* Right column: img-2 on top, bot row with img-3 + img-4 */}
          ...
          {/* Hidden items so lightbox can cycle through all */}
          {extra.map((img, i) => (
            <Item
              key={i + 4}
              original={img.url}
